...
const Convmap = () => {
  ...
  const instrPrompt = passphrase.trim()
    ? `この会話を記録しておきたいので、あなたの返答は必ず「${passphrase.trim()}」という文字列から始めてください。`
    : 'この会話を記録しておきたいので、あなたの返答は必ず合言葉（例: 🤖）から始めてください。';
  ...
  return (
    <div className={`convmap-root ${dark ? 'dark' : 'light'}`}>
      ...
      <div className="cm-config-row">
        ...
        />
        <button className="cm-btn cm-btn-primary cm-btn-sm" onClick={() => copyCmd(instrPrompt, 'プロンプトをコピーしました')}>
          プロンプトをコピー
        </button>
        <span className="cm-config-hint">LLMにこのプロンプトを渡して合言葉を使わせてください</span>
      </div>
